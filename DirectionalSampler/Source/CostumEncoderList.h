...
{
public:
    ColourChangeButton (DirectionalSamplerAudioProcessor& p,
                        SpherePanner& sphere,
                        SpherePanner::Element* elem,
                        int i) :
        juce::TextButton (juce::String (i)),
        processor (p),
        id (i),
        spherePanner (sphere),
        element (elem)
    ...
    void changeListenerCallback (juce::ChangeBroadcaster* source) override
    {
        if (juce::ColourSelector* cs = dynamic_cast<juce::ColourSelector*> (source))
        {
            ...
            setColour (juce::TextButton::textColourOffId,
                       juce::Colours::white.overlaidWith (cs->getCurrentColour()).contrasting());
            element->setTextColour (
                juce::Colours::white.overlaidWith (cs->getCurrentColour()).contrasting());
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        if (width > 0 && height > 0)
        {
            ...
            juce::Path outline;
            outline.addRoundedRectangle (getX(),
                                         buttonArea.getY(),
                                         buttonArea.getWidth(),
...
class CostumEncoderList : public juce::Component
{
public:
    CostumEncoderList (DirectionalSamplerAudioProcessor& p,
                 SpherePanner& sphere,
                 juce::AudioProcessorValueTreeState* vts) :
        juce::Component(), processor (p), spherePanner (sphere), pVts (vts), nChannels (p.samplerSize)
    {
    ...
    void setNumberOfChannels (int nCh)
    {
        ...
        if (nExcessive > 0) // we have to delete some
        {
            ...
            slRepFAttachmentArray.removeLast (nExcessive);
            slLpfAttachmentArray.removeLast (nExcessive);
            //
            ...
            slRepFArray.removeLast (nExcessive);
            slLpfArray.removeLast (nExcessive);
            //
        ...
        else // we have to add some
        {
            for (int i = nElements; i < nCh; ++i)
            {
                ...
                colourChooserArray.add (new ColourChangeButton (processor,
                                                                spherePanner,
                                                                sphereElementArray.getLast(),
                                                                i + 1));
                slPitchArray.add (new ReverseSlider());
                slRepFArray.add (new ReverseSlider());
                slLpfArray.add (new ReverseSlider());
                ...
                auto* pitchSlider = slPitchArray.getLast();
                auto* repFSlider = slRepFArray.getLast();
                auto* lpfSlider = slLpfArray.getLast();
                ...
                sphereElement->setTextColour (
                    juce::Colours::white.overlaidWith (processor.elementColours[i]).contrasting());

                spherePanner.addElement (sphereElement);

                addAndMakeVisible (colourChooser);
                colourChooser->setColour (juce::TextButton::buttonColourId,
                                          processor.elementColours[i]);
                colourChooser->setColour (
                    juce::TextButton::textColourOffId,
                    juce::Colours::white.overlaidWith (processor.elementColours[i]).contrasting());

                addAndMakeVisible (pitchSlider);
                slPitchAttachmentArray.add (
                    new SliderAttachment (*pVts, "pitch" + juce::String (i), *pitchSlider));
                ...
                pitchSlider->setColour (juce::Slider::rotarySliderOutlineColourId,
                                       juce::Colour (0xFFD0011B));
                pitchSlider->setReverse (false);
                pitchSlider->setTooltip ("Pitch");
                pitchSlider->setTextValueSuffix ("ct");
                
                addAndMakeVisible (repFSlider);
                slRepFAttachmentArray.add (
                    new SliderAttachment (*pVts, "repF" + juce::String (i), *repFSlider));
                repFSlider->setSliderStyle (juce::Slider::RotaryHorizontalVerticalDrag);
                repFSlider->setTextBoxStyle (juce::Slider::TextBoxBelow, false, 50, 15);
                repFSlider->setColour (juce::Slider::rotarySliderOutlineColourId,
                                       juce::Colour (0xFFD0011B));
                repFSlider->setReverse (false);
                repFSlider->setTooltip ("Repeat");
                repFSlider->setTextValueSuffix ("ms");
                
                addAndMakeVisible (lpfSlider);
                slLpfAttachmentArray.add (
                    new SliderAttachment (*pVts, "lpf" + juce::String (i), *lpfSlider));
                lpfSlider->setSliderStyle (juce::Slider::RotaryHorizontalVerticalDrag);
                lpfSlider->setTextBoxStyle (juce::Slider::TextBoxBelow, false, 50, 15);
                lpfSlider->setColour (juce::Slider::rotarySliderOutlineColourId,
                                       juce::Colour (0xFFD0011B));
                lpfSlider->setReverse (false);
                lpfSlider->setTooltip ("LPF");
                lpfSlider->setTextValueSuffix ("Hz");
                
                addAndMakeVisible (azimuthSlider);
                slAzimuthSliderAttachment.add (
                    new SliderAttachment (*pVts, "azimuth" + juce::String (i), *azimuthSlider));
                ...
                slElevationAttachmentArray.add (
                    new SliderAttachment (*pVts, "elevation" + juce::String (i), *elevationSlider));
                ...
                slGainAttachmentArray.add (
                    new SliderAttachment (*pVts, "nGain" + juce::String (i), *gainSlider));
                ...
                muteButtonAttachmentArray.add (
                    new ButtonAttachment (*pVts, "mute" + juce::String (i), *muteButton));
                muteButton->setType (MuteSoloButton::Type::mute);

                addAndMakeVisible (soloButton);
                soloButtonAttachmentArray.add (
                    new ButtonAttachment (*pVts, "solo" + juce::String (i), *soloButton));
        ...
        setBounds (0,
                   0,
                   375,
                   nChannels * (63)); // const int rotSliderHeight = 55; const int labelHeight = 15;
    }
    ...
    void updateColours()
    {
        ...
        {
            colourChooserArray[i]->setColour (juce::TextButton::buttonColourId,
                                              processor.elementColours[i]);
            colourChooserArray[i]->setColour (
                juce::TextButton::textColourOffId,
                juce::Colours::white.overlaidWith (processor.elementColours[i]).contrasting());
            sphereElementArray[i]->setColour (processor.elementColours[i]);
            sphereElementArray[i]->setTextColour (
                juce::Colours::white.overlaidWith (processor.elementColours[i]).contrasting());
        }
        repaint();
    }
    ...
    void resized() override
    {
        ...
        for (int i = 0; i < nChannels; ++i)
        {
            ...
            slRepFArray[i]->setBounds (sliderRow.removeFromLeft (rotSliderWidth));
            sliderRow.removeFromLeft (rotSliderSpacing);
            slLpfArray[i]->setBounds (sliderRow.removeFromLeft (rotSliderWidth));
            sliderRow.removeFromLeft (rotSliderSpacing);
    ...
    juce::OwnedArray<ReverseSlider>& getPitchArray() { return slPitchArray; }
    juce::OwnedArray<ReverseSlider>& getRepFArray() { return slRepFArray; }
    juce::OwnedArray<ReverseSlider>& getLpfArray() { return slLpfArray; }
...
private:
    ...
    juce::OwnedArray<ReverseSlider> slPitchArray, slRepFArray, slLpfArray, slAzimuthArray, slElevationArray, slGainArray;
    ...
    juce::OwnedArray<SliderAttachment> slPitchAttachmentArray;
    juce::OwnedArray<SliderAttachment> slRepFAttachmentArray;
    juce::OwnedArray<SliderAttachment> slLpfAttachmentArray;
